...
export default function Home() {
  ...
  const fetchEvents = async () => {
    try {
      const { data } = await api.get('/events?status=approved') // Only show approved events
      setEvents(data || [])
    } catch (error) {
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <section id="home" className="min-h-screen flex items-center justify-center pt-20 px-4">
        <div className="max-w-6xl mx-auto text-center">
          ...
          {/* New Stats Section */}
          <div className="grid grid-cols-2 gap-8 max-w-2xl mx-auto mt-16 border-t border-gray-800 pt-8">
            <div className="text-center">
              <div className="text-4xl font-bold text-white mb-2">{stats?.teachers || 0}</div>
              <div className="text-gray-500 text-sm uppercase tracking-wider">Faculty Mentors</div>
            </div>
            <div className="text-center">
              <div className="text-4xl font-bold text-white mb-2">{stats?.students || 0}</div>
              <div className="text-gray-500 text-sm uppercase tracking-wider">Active Students</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-4 bg-gray-900/50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-gray-400 text-lg">Mark your calendars for our next big activities</p>
          </div>

          {events.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {events.slice(0, 3).map(event => (
                <div key={event.id} className="bg-gray-900 border border-gray-800 rounded-xl p-6 hover:border-green-500 transition group">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-12 h-12 bg-green-900/50 rounded-lg flex items-center justify-center text-green-400 font-bold text-lg border border-green-800">
            ...
            <div className="text-center py-12 bg-gray-900 border border-gray-800 rounded-xl">
              ...
              <p className="text-gray-500">More exciting events coming soon!</p>
            </div>
          )}

          <div className="text-center mt-10">
            <Link to="/events" className="text-blue-400 hover:text-blue-300 font-medium inline-flex items-center gap-2">
