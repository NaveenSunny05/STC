...
router.post('/direct', auth, async (req, res) => {
  try {
    ...
    try {
      const io = getIo();
      io.to(receiver_id).emit('new_dm', data);
      console.log(`📡 Emitted new_dm to ${receiver_id}`);
    } catch (socketError) {
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    try {
      const io = getIo();
      io.to(channel_id).emit('new_channel_message', formattedMessage);
      console.log(`📡 Emitted new_channel_message to ${channel_id}`);
    } catch (socketError) {
      console.error('⚠️ Socket emit error:', socketError.message);
    }
